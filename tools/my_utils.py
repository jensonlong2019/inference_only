import platform, os, shutil, traceback
import ffmpeg
import numpy as np
import gradio as gr
...
def _ffmpeg_install_hint():
    if platform.system() == "Darwin":
        return "请安装 ffmpeg: bash install_ffmpeg_mac.sh（无需 brew）或 brew install ffmpeg"
    if platform.system() == "Windows":
        return "请安装 ffmpeg 并加入 PATH: https://ffmpeg.org/download.html"
    return "请安装 ffmpeg: sudo apt install ffmpeg  或  sudo yum install ffmpeg"
...
def load_audio(file, sr):
    ...
    traceback.print_exc()
    raise RuntimeError(
        i18n("音频加载失败") + f"。{_ffmpeg_install_hint()}。详情: {'; '.join(errors)}"
    )
