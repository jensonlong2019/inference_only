...
def _posseg_pair_to_tuple(item):
    if hasattr(item, "word") and hasattr(item, "flag"):
        return (item.word, item.flag)
...
def _posseg_lcut(text):
    """jieba.posseg 旧版只有 cut；jieba_fast 提供 lcut。"""
    if hasattr(psg, "lcut"):
        seg = psg.lcut(text)
    else:
        seg = list(psg.cut(text))
    return [_posseg_pair_to_tuple(item) for item in seg]
